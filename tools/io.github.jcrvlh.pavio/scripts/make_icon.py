...
BOMB_BODY   = (0xC6, 0x47, 0x2F, 255)
FUSE_NECK   = (0x3A, 0x3A, 0x3E, 255)
FUSE_STEM   = (0xE8, 0xE8, 0xE8, 255)
SPARK_COLOR = (0xE9, 0xB2, 0x3C, 255)
...
def _sample(px, py):
    ...
    # Faísca (quadrado rotacionado 45° em (120, 52))
    dx = px - 120.0
    dy = py - 52.0
    rx = (dx + dy) * 0.70710678
    ry = (-dx + dy) * 0.70710678
    if _round_rect_sdf(rx, ry, -8.0, -8.0, 16.0, 16.0, 3.0) <= 0:
        return SPARK_COLOR

    # Pavio (haste)
    if _round_rect_sdf(px, py, 117.0, 60.0, 6.0, 24.0, 2.0) <= 0:
        return FUSE_STEM

    # Pescoço da bomba
    if _round_rect_sdf(px, py, 113.0, 82.0, 14.0, 8.0, 2.0) <= 0:
        return FUSE_NECK

    # Corpo redondo da bomba
    if math.hypot(px - 120.0, py - 138.0) <= 50.0:
        return BOMB_BODY
