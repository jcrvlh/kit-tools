#!/usr/bin/env python3
"""
make_icon.py — rasteriza icon.svg para Quebra-Gelo:
...
BALLOON = (44.0, 60.0, 152.0, 96.0, 28.0)
STROKE = 3.0

CARD_FILL     = (0x0A, 0x0A, 0x0B, 255)
CARD_BORDER   = (0x3A, 0x3A, 0x3E, 255)
BALLOON_COLOR = (0x2C, 0x3C, 0xC4, 255)
DOT_COLOR     = (0xEF, 0xEA, 0xDD, 255)
...
def _triangle_sdf(px, py, p0, p1, p2):
    # Triângulo retângulo simples entre (68, 154), (68, 186), (98, 154)
    # x >= 68, y >= 154, e abaixo da hipotenusa (dx/30 + dy/32 <= 1)
    if px >= 68.0 and py >= 154.0:
        u = (px - 68.0) / 30.0
        v = (py - 154.0) / 32.0
        if u + v <= 1.0:
            return -1.0
...
def _sample(px, py):
    ...
    # Três pontos (reticências) no centro do balão
    for cx in (88.0, 120.0, 152.0):
        if math.hypot(px - cx, py - 108.0) <= 10.0:
            return DOT_COLOR

    # Balão e cauda
    if _round_rect_sdf(px, py, *BALLOON) <= 0 or _triangle_sdf(px, py, None, None, None) <= 0:
