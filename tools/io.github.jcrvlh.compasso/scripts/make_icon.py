#!/usr/bin/env python3
"""
make_icon.py — rasteriza o ícone do Compasso em:

  icon.png   240x240 RGBA   — catálogo web / previews
  icon.bin    64x64  LVGL v9 (ARGB8888) — asset do pacote .kit

Um mostrador: anel verde (a cor base da Tool) + ponteiro apontando pra ~2h
+ pino central. Sobre a carta preta padrão do catálogo.
...
DIAL_COLOR  = (0x45, 0xA0, 0x5B, 255)           # KIT_COLOR_GREEN
...
HAND_LEN = 50.0
HAND_W = 13.0
HAND_ANG = math.radians(-54.0)                  # ~2 horas (0 = 3h, sobe anti-horário)
PIN_R = 12.0
...
def _segment_sdf(px, py, ax, ay, bx, by):
    pax, pay = px - ax, py - ay
    bax, bay = bx - ax, by - ay
...
def _sample(px, py):
    """Cor RGBA (a=0 fora da carta) para um ponto no espaço 240."""
    ...
    if d <= PIN_R:
        return DIAL_COLOR
    # ponteiro
    hx = cx + HAND_LEN * math.cos(HAND_ANG)
    hy = cy + HAND_LEN * math.sin(HAND_ANG)
    if _segment_sdf(px, py, cx, cy, hx, hy) <= HAND_W / 2:
        return DIAL_COLOR
