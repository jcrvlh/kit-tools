#!/usr/bin/env python3
"""
make_icon.py — rasteriza o ícone da Soundbox (geometria abaixo) em:

  icon.png   240x240 RGBA   — catálogo web / previews
  icon.bin    64x64  LVGL v9 (ARGB8888) — asset do pacote .kit

Uma grade 3x3 de pads sobre o cartão, no espírito do glifo TOOL_ICON_SOUNDBOX
do Launcher. Sem dependências: renderer por supersampling + encoder PNG stdlib.
...
PAD_GREEN   = (0x45, 0xA0, 0x5B, 255)   # KIT_COLOR_GREEN
PAD_YELLOW  = (0xE9, 0xB2, 0x3C, 255)   # KIT_COLOR_YELLOW
PAD_RED     = (0xC6, 0x47, 0x2F, 255)   # KIT_COLOR_RED

# Grade 3x3 centrada em (120, 120): pads de 48 px, gap de 14 px.
PAD_SIZE = 48.0
PAD_GAP = 14.0
PAD_R = 10.0
GRID0 = 120.0 - 1.5 * PAD_SIZE - PAD_GAP     # canto do pad (0,0)

# Uma pitada de cor: a maioria verde, um amarelo e um vermelho.
PAD_COLORS = {
    (2, 0): PAD_YELLOW,
    (0, 2): PAD_RED,
}
...
def _sample(px, py):
    ...
    for row in range(3):
        for col in range(3):
            x = GRID0 + col * (PAD_SIZE + PAD_GAP)
            y = GRID0 + row * (PAD_SIZE + PAD_GAP)
            if _round_rect_sdf(px, py, x, y, PAD_SIZE, PAD_SIZE, PAD_R) <= 0:
                return PAD_COLORS.get((col, row), PAD_GREEN)
