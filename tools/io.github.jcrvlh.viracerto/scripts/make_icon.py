#!/usr/bin/env python3
"""
make_icon.py — rasteriza o ícone do Vira Certo (geometria replicada abaixo) em:

  icon.png   240x240 RGBA   — catálogo web / previews
  icon.bin    64x64  LVGL v9 (ARGB8888) — asset do pacote .kit

Geometria: mesma do TOOL_ICON_DIAL do Launcher — anel + ponteiro apontando
pro alvo (canto superior direito) + pivô central.
...
DIAL_COLOR  = (0x2C, 0x3C, 0xC4, 255)   # KIT_COLOR_BLUE
...
# Ponteiro: barra do centro pro alvo, 34° acima da horizontal (canto sup. dir.).
POINTER_ANGLE = math.radians(-34.0)
POINTER_LEN = 84.0
POINTER_HALF_W = 7.0
...
def _sample(px, py):
    ...
    # Anel
    if abs(dist - RING_R) <= RING_STROKE / 2:
        return DIAL_COLOR

    # Ponteiro (segmento do centro pra fora, ao longo de POINTER_ANGLE)
    ca, sa = math.cos(POINTER_ANGLE), math.sin(POINTER_ANGLE)
    along = dx * ca + dy * sa
    perp = -dx * sa + dy * ca
    if 0.0 <= along <= POINTER_LEN and abs(perp) <= POINTER_HALF_W:
        return DIAL_COLOR
