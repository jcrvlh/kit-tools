#!/usr/bin/env python3
"""
make_icon.py — rasteriza icon.svg para Veto:
...
TARGET = (40.0, 52.0, 160.0, 42.0, 8.0)
FOUL_SQ1 = (44.0, 120.0, 22.0, 22.0, 4.0)
FOUL_BAR1 = (76.0, 124.0, 120.0, 14.0, 4.0)
FOUL_SQ2 = (44.0, 156.0, 22.0, 22.0, 4.0)
FOUL_BAR2 = (76.0, 160.0, 96.0, 14.0, 4.0)
STROKE = 3.0

CARD_FILL    = (0x0A, 0x0A, 0x0B, 255)
CARD_BORDER  = (0x3A, 0x3A, 0x3E, 255)
TARGET_COLOR = (0xE9, 0xB2, 0x3C, 255)
FOUL_COLOR   = (0xC6, 0x47, 0x2F, 255)
TEXT_COLOR   = (0xEF, 0xEE, 0xEE, 255)
...
def _sample(px, py):
    """Cor opaca RGBA para um ponto no espaço 240."""
    if _round_rect_sdf(px, py, *CARD) > 0:
        return (0, 0, 0, 0)

    # Palavra-alvo
    if _round_rect_sdf(px, py, *TARGET) <= 0:
        return TARGET_COLOR

    # Proibida 1
    if _round_rect_sdf(px, py, *FOUL_SQ1) <= 0:
        return FOUL_COLOR
    if _round_rect_sdf(px, py, *FOUL_BAR1) <= 0:
        return TEXT_COLOR

    # Proibida 2
    if _round_rect_sdf(px, py, *FOUL_SQ2) <= 0:
        return FOUL_COLOR
    if _round_rect_sdf(px, py, *FOUL_BAR2) <= 0:
        return TEXT_COLOR
