...
STROKE = 3.0
SQUARES = (
    (68.0, 52.0, 46.0, 46.0, 6.0),
    (126.0, 52.0, 46.0, 46.0, 6.0),
    (68.0, 110.0, 46.0, 46.0, 6.0),
    (126.0, 110.0, 46.0, 46.0, 6.0),
)
CIRCLE = (120.0, 196.0, 24.0)                    # cx, cy, r

CARD_FILL   = (0x0A, 0x0A, 0x0B, 255)
CARD_BORDER = (0x3A, 0x3A, 0x3E, 255)
C_SQUARE    = (0xE8, 0xE8, 0xE8, 255)
C_CIRCLE    = (0xC6, 0x47, 0x2F, 255)
...
def _sample(px, py):
    ...
    for sq in SQUARES:
        if _round_rect_sdf(px, py, *sq) <= 0:
            return C_SQUARE
    cx, cy, cr = CIRCLE
    if math.hypot(px - cx, py - cy) <= cr:
        return C_CIRCLE
    return col
